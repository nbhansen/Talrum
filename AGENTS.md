...
- When you name a cloud product or vendor term ("Speech resource", "deployment", "region"), say in plain words what the user must do and what they get. Vendor nouns are not STE.
...
- AI review handling: a `REQUEST_CHANGES` review blocks — fix it in the PR. A `COMMENT` review ("No blocking concerns") permits merge — do not push commits to answer its inline notes. Reply on each thread, file a `gh issue` for each note that is real, and merge.
- Never add a new mechanism (counter, budget, guard, cap, fallback) to an open PR because a review comment suggested it. That is new scope. New scope gets its own issue and its own PR off `main`. (PR #460: one guard fix grew eight review-fix rounds in one hour because each advisory got a same-PR fix commit.)
